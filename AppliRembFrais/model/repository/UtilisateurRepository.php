...
class UtilisateurRepository extends Repository
{
    ...
    public function getMonUtilisateur($idUtilConnecte)
    {
        $db = $this->dbConnect();
        $req = $db->prepare("select nom, prenom, pseudo, profil.libelle from utilisateur join profil on profil.id = id_profil 
        where utilisateur.id = :par_id_util_connect");
        $req->bindValue(":par_id_util_connect", $idUtilConnecte, PDO::PARAM_INT);
        // execution request
        $req->execute();
        $enreg = $req->fetch();
        $unUtilisateur = new Utilisateur(
