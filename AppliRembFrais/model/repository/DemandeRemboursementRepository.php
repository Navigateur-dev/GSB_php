...
class DemandeRemboursementRepository extends Repository
{
    ...
    public function getLesDemandesRemboursement()
    {
        ...
        $req = $db->prepare("select demande_remboursement.id as id,
                        DATE_FORMAT(date_saisie, '%d/%m/%Y à %H:%i:%s') as date_saisie, 
                        type_frais.libelle,montant, commentaire, utilisateur.nom, utilisateur.prenom
                        from demande_remboursement 
                join type_frais on type_frais.id = id_type_frais join utilisateur on id_delegue = utilisateur.id");
        // on demande l'exécution de la requête 
        $req->execute();
        $lesEnregs = $req->fetchAll();
        foreach ($lesEnregs  as $enreg) {
            $uneDemande = new DemandeRemboursement(
                ...
                $enreg->commentaire,
                new TypeFrais(null, $enreg->libelle),
                new Utilisateur(null, $enreg->nom, $enreg->prenom)
            );
            array_push($lesDemandes, $uneDemande);
        }
        return $lesDemandes;
